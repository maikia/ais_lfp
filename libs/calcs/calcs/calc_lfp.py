...
def get_lfp_data(data, calc_lfp_new, field_size_type,run_original):
    if run_original:
        prefix = 'physiol'
    else:
        prefix = 'reduced'
    ...
    if field_size_type == 'zoom':
        ...
        savename = prefix+'zoom'
    elif field_size_type == 'full':
        n_samp = 5  # grid size will be n_samp x n_samp
        record_x_range = [-300, 400]
        record_y_range = [-300, 1200]
        savename = prefix+'full'
    elif field_size_type == 'detail':
        n_samp = 200  # grid size will be n_samp x n_samp
        record_x_range = [-400, 800]
        record_y_range = [-800, 1200]
        savename = prefix+'detail'
    elif field_size_type == 'ais_det':
        n_samp = 150  # grid size will be n_samp x n_samp
        record_x_range = [-70, 70]
        record_y_range = [-150, 100]
        savename = prefix+'ais_det'
    elif field_size_type == 'ais_zoom':
        n_samp = 150  # grid size will be n_samp x n_samp
        record_x_range = [-50, 50]
        record_y_range = [-100, 20]
        savename = prefix+'ais_zoom'
    elif field_size_type == 'lfp_zoom_out':
        n_samp = 200  # grid size will be n_samp x n_samp
        record_x_range = [-400, 800]
        record_y_range = [-800, 1200]
        savename = prefix+'lfp_zoom_ou'
    else:
        raise ValueError('field_size_type has a wrong value')

    if calc_lfp_new:
        calc_field(data,record_x_range,record_y_range,n_samp,savename)

    lfp_data = np.load(savename+'.npz')
    return lfp_data
...
if __name__ == '__main__':
    ...
    simulate_new = False
    run_original = False # if False, will rund reduced_Nav
